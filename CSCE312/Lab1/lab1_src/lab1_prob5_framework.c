...
#define DRIVER_SEAT_BELT 0  // bit 0
#define ENGINE_RUNNING   1  // bit 1
#define DOORS_CLOSED     2  // bit 2
#define DOOR_LOCK_LEVER  3  // bit 3
#define DRIVER_ON_SEAT   4  // bit 4
#define KEY_IN_CAR       5  // bit 5
#define BRAKE_PEDAL      6  // bit 6
#define CAR_MOVING       7  // bit 7
...
void control_action(){
    ...
    */
    ...
    if((input & (1 << ENGINE_RUNNING)) 
    && (!(input & (1 << DRIVER_SEAT_BELT)) || !(input & (1 << DOORS_CLOSED)))){
        output = output | 1;
    }
    if((input & ((1 << DRIVER_ON_SEAT) + (1 << DOOR_LOCK_LEVER))) 
    == (1 << DRIVER_ON_SEAT) + (1 << DOOR_LOCK_LEVER)){
        output = output | 2;
    }
    if((input & ((1 << BRAKE_PEDAL) + (1 << CAR_MOVING))) 
    == (1 << BRAKE_PEDAL) + (1 << CAR_MOVING)){
        output = output | 4;
    }
}
